class FacebookController < ApplicationController
  ...
  def index
    unless current_user.facebook_oauth_setting
      @oauth = Koala::Facebook::OAuth.new("605810012807998",
                                          "7051fd1d664fece11337afb97459e9e7",
                                          "http://#{request.host}:#{request.port}/callback")
  ...
  def callback
    unless current_user.facebook_oauth_setting
      ...
      FacebookOauthSetting.create({:access_token => @oauth.get_access_token(params[:code]),
